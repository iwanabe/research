...
class MeshIO {
	...
	static writeSTL(polygonSoup){
		let output = "";
		
		let positions = polygonSoup["v"];
		let uvs = polygonSoup["vt"];
		let normals = polygonSoup["vn"];
		var indices = polygonSoup["f"];
		...
		for (let i = 0; i < normals.length/3; i++) {
			if (normals) output += "\tfacet normal " + normals[3 * i + 0] + " " + normals[3 * i + 1] + " " + normals[3 * i + 2] + "\n";
			output += '\t\touter loop\n';
			
			for ( var j = 0; j < 3; j ++ ) {
				//var vertexIndex = indices[ j ];
				//vector.copy( positions[ vertexIndex ] ).applyMatrix4( matrixWorld );
				//output += '\t\t\tvertex ' + vector.x + ' ' + vector.y + ' ' + vector.z + '\n';
				var vertexIndex0 = indices[ 3 * i + j ];
				var vertexIndex1 = indices[ 3 * i + j + 1 ];
				var vertexIndex2 = indices[ 3 * i + j + 2 ];
				
				output += '\t\t\tvertex ' + positions[ vertexIndex0 ] + ' ' + positions[ vertexIndex1 ] + ' ' + positions[ vertexIndex2 ] + '\n';
			}
